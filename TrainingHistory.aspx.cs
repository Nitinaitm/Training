...
namespace Training
{
    public partial class TrainingHistory : Page
    {
        ...
        {
            get { return Request.QueryString["TrainingID"]; }
        }
        ...
        {
            get { return Session["EmpID"] == null ? "" : Session["EmpID"].ToString().Trim().ToUpperInvariant(); }
        }

        private string TrainerID
        {
            get { return Session["TrainerID"] == null ? "" : Session["TrainerID"].ToString().Trim(); }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(EmpID) || !HasTraineeAccess())
                {
                    GoDashboard();
                    return;
                }
            ...
            {
                if (string.IsNullOrWhiteSpace(TrainerID) || !HasTrainerAccess())
                {
                    GoDashboard();
                    return;
                }
        ...
        {
            object value = obj.ExecuteScalar("SELECT COUNT(*) FROM TrainingAssignment WHERE TrainingID=@TrainingID AND EmpID=@EmpID AND AssignmentStatus='Assigned'", new SqlParameter[]
            {
        ...
        {
            object value = obj.ExecuteScalar("SELECT COUNT(*) FROM SessionMaster WHERE TrainingID=@TrainingID AND TrainerID=@TrainerID", new SqlParameter[]
            {
                new SqlParameter("@TrainingID", TrainingID),
        ...
        {
            object value = obj.ExecuteScalar("SELECT COUNT(*) FROM TrainingDetails WHERE TrainingID=@TrainingID AND TrainingStatus='Closed'", new SqlParameter[]
            {
                new SqlParameter("@TrainingID", TrainingID)
            });
            return value != null && Convert.ToInt32(value) > 0;
        ...
        private void BindAttendance()
        {
            ...
            if (string.Equals(Role, "Emp", StringComparison.OrdinalIgnoreCase))
            {
                sql = "SELECT SM.SessionNo,SM.SessionName,SM.SessionDate,ISNULL(SA.AttendanceStatus,'Not Marked') AS AttendanceStatus,CONVERT(varchar(16),SM.AttendanceCompletedOn,105) AS AttendanceCompletedOn,ISNULL(SM.TrainerID,'') AS TrainerName FROM SessionMaster SM LEFT JOIN SessionAttendance SA ON SA.SessionID=SM.SessionID AND SA.EmpID=@EmpID WHERE SM.TrainingID=@TrainingID ORDER BY TRY_CONVERT(date,SM.SessionDate,105),TRY_CONVERT(int,SM.SessionNo)";
                parameters = new SqlParameter[] { new SqlParameter("@TrainingID", TrainingID), new SqlParameter("@EmpID", EmpID) };
            }
            else
            {
                sql = "SELECT SM.SessionNo,SM.SessionName,SM.SessionDate,ISNULL(SM.AttendanceStatus,'Pending') AS AttendanceStatus,CONVERT(varchar(16),SM.AttendanceCompletedOn,105) AS AttendanceCompletedOn,SM.TrainerID AS TrainerName FROM SessionMaster SM WHERE SM.TrainingID=@TrainingID AND SM.TrainerID=@TrainerID ORDER BY TRY_CONVERT(date,SM.SessionDate,105),TRY_CONVERT(int,SM.SessionNo)";
        ...
        private void BindTests()
        {
            ...
            if (string.Equals(Role, "Emp", StringComparison.OrdinalIgnoreCase))
            {
                sql = @"SELECT TM.TestType,TM.TestTitle,SM.SessionName,
                        CONVERT(varchar(10),R.SubmittedOn,105) AS TestDate,
                        ROW_NUMBER() OVER(PARTITION BY TM.TestID,R.EmpID ORDER BY R.SubmittedOn) AS AttemptNo,
                        R.TotalQuestions,R.CorrectAnswers,R.Score,R.Status,
                        CONVERT(varchar(16),R.SubmittedOn,105) AS SubmittedOn
                        FROM TestResult R
        ...
        private void BindQuestions()
        {
            ...
            if (string.Equals(Role, "Emp", StringComparison.OrdinalIgnoreCase))
            {
                sql = @"SELECT TM.TestType,TM.TestTitle,
                        ROW_NUMBER() OVER(PARTITION BY TA.ResultID ORDER BY TA.SequenceNo) AS AttemptNo,
                        TA.SequenceNo AS QuestionNo,QB.Question,TA.SelectedAnswer,
                        QB.Answer AS CorrectAnswer,
                        CASE WHEN ISNULL(TA.IsCorrect,0)=1 THEN 'Correct' ELSE 'Incorrect' END AS IsCorrect
                        FROM TestAttempt TA
                        INNER JOIN TestResult R ON R.ResultID=TA.ResultID
                        INNER JOIN TestMaster TM ON TM.TestID=R.TestID
                        INNER JOIN SessionMaster SM ON SM.SessionID=TM.SessionID
        ...
        private void BindFeedback()
        {
            ...
            if (string.Equals(Role, "Emp", StringComparison.OrdinalIgnoreCase))
            {
                sql = @"SELECT CONVERT(varchar(16),F.SubmittedOn,105) AS SubmittedOn,
                        ISNULL(FCM.CategoryName,'') AS CategoryName,FQ.QuestionText,
                        FD.AnswerType,FD.Rating,FD.Answer,ISNULL(SM.SessionName,'') AS SessionName,
                        ISNULL(FD.TrainerID,'') AS TrainerName
                        FROM Feedback F
                        INNER JOIN FeedbackDetail FD ON FD.FeedbackID=F.FeedbackID
                        LEFT JOIN FeedbackCategoryMaster FCM ON FCM.CategoryID=FD.CategoryID
                        LEFT JOIN FeedbackQuestionMaster FQ ON FQ.QuestionID=FD.QuestionID
            ...
            else
            {
                sql = @"SELECT CONVERT(varchar(16),F.SubmittedOn,105) AS SubmittedOn,
                        ...
                        AND EXISTS (SELECT 1 FROM SessionMaster SM2 WHERE SM2.TrainingID=F.TrainingID AND SM2.TrainerID=@TrainerID)
        ...
        private void SetCertificateButton()
        {
            ...
            object value = obj.ExecuteScalar("SELECT COUNT(*) FROM TrainingCertificate WHERE TrainingID=@TrainingID AND EmpID=@EmpID AND CertificateStatus='A'", new SqlParameter[]
            {
                new SqlParameter("@TrainingID", TrainingID),
                new SqlParameter("@EmpID", EmpID)
            });
        ...
        private void GoDashboard()
        {
            ...
            else if (string.Equals(Role, "Emp", StringComparison.OrdinalIgnoreCase))
                Response.Redirect("~/Trainee/Default.aspx");
            else
