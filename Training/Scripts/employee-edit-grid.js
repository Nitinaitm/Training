(function () {
    function addEmployeeEditButtons() {
        ...
        if (!header.querySelector('.employee-edit-header')) {
            ...
            th.className = 'employee-edit-header';
            th.innerHTML = 'Action';
        ...
        for (var i = 1; i < rows.length; i++) {
            ...
            if (row.querySelector('.employee-edit-link')) continue;
    ...
    function openEmployeeEditor(empID) {
        ...
        var html = '' +
            ...
            '   </div>' +
            '   <div class="modal-body p-0" style="height:520px;overflow:hidden;">' +
            '    <iframe src="Admin/EditEmployee.aspx?EmpID=' + empID + '" style="width:100%;height:100%;border:0;"></iframe>' +
            '   </div>' +
            '  </div>' +
            ' </div>' +
            '</div>';
    ...
    document.addEventListener('click', function (e) {
        var button = e.target.closest ? e.target.closest('.employee-edit-link') : null;
        if (!button) return;
        e.preventDefault();
        openEmployeeEditor(button.getAttribute('data-empid'));
    });
