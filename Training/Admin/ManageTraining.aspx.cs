...
AND NOT EXISTS (
 ...
 AND NOT EXISTS (SELECT 1 FROM TestMaster TM INNER JOIN TestAttempt TA ON TA.TestID=TM.TestID WHERE TM.SessionID=S.SessionID AND TM.TestType=@TestType AND TM.IsPublished=1 AND TA.EmpID=A.EmpID AND TA.Submitted=1)
            ...
            SqlParameter[] p = { new SqlParameter("@TrainingID", TrainingID), new SqlParameter("@TestType", testType) };
            return Convert.ToInt32(new clsDataAccess().ExecuteScalar(q, p)) == 1;
        }
...
AND NOT EXISTS (SELECT 1 FROM TrainingAssignment A WHERE A.TrainingID=@TrainingID AND NOT EXISTS (SELECT 1 FROM TrainingCertificate C WHERE C.TrainingID=@TrainingID AND C.EmpID=A.EmpID AND C.CertificateStatus='A')) THEN 1 ELSE 0 END";
        ...
        {
            bool fr=IsFeedbackRequired(), cr=IsCertificateRequired(), ar=IsAttendanceRequired(), pr=IsPreTestRequired(), por=IsPostTestRequired();
            bool ta=IsTraineeAssigned(), sa=HasSessionsAndTrainers();
            bool fa=!fr||IsFeedbackAssigned(), ct=!cr||IsCertificateTemplateConfigured(), ac=!ar||AreAllAttendanceCompleted();
            bool pc=!pr||AreAllTestsCompleted("Pre"), poc=!por||AreAllTestsCompleted("Post"), fs=!fr||IsFeedbackSubmitted(), cg=!cr||IsCertificateGenerated();
            int n=1; StringBuilder h=new StringBuilder();
            h.Append(Stage("Create Batch",true,true,ref n));
            h.Append(Stage("Assign Sessions & Trainers",true,sa,ref n));
            h.Append(Stage("Assign Trainees",true,ta,ref n));
            h.Append(Stage("Feedback Assigned",fr,fa,ref n));
            h.Append(Stage("Certificate Template",cr,ct,ref n));
            h.Append(Stage("Attendance Completed",ar,ac,ref n));
            h.Append(Stage("Pre-Test Completed",pr,pc,ref n));
            h.Append(Stage("Post-Test Completed",por,poc,ref n));
            h.Append(Stage("Feedback Submitted",fr,fs,ref n));
            h.Append(Stage("Certificate Generated",cr,cg,ref n));
        ...
        private void LoadWorkflow()
        {
            ...
            using(SqlConnection con=new SqlConnection(constr))
            {
                ...
                cmd.Parameters.AddWithValue("@TrainingID",TrainingID); con.Open(); SqlDataReader dr=cmd.ExecuteReader(); if(!dr.Read()) return;
                ...
                bool th=dr["TrainerHostelRequired"]!=DBNull.Value&&Convert.ToBoolean(dr["TrainerHostelRequired"]); bool trh=dr["TraineeHostelRequired"]!=DBNull.Value&&Convert.ToBoolean(dr["TraineeHostelRequired"]); bool legacy=string.Equals(dr["HostelRequiredTrainee"].ToString(),"Yes",StringComparison.OrdinalIgnoreCase); hostelRequired=th||trh||legacy; dr.Close();
            }
            bool ta=IsTraineeAssigned(), sa=HasSessionsAndTrainers(), fr=IsFeedbackRequired(), fa=!fr||IsFeedbackAssigned(), ct=!IsCertificateRequired()||IsCertificateTemplateConfigured(), ac=!IsAttendanceRequired()||AreAllAttendanceCompleted();
            btnUpdateTraining.Visible=true; btnAssignSession.Visible=true; btnAssignTrainee.Visible=true; btnRequirements.Visible=true;
            ...
            btnCertificateTemplate.Visible=certificateRequired&&!GetRequirement("CertificateSkipped"); btnCertificateTemplate.Enabled=certificateRequired&&ta&&!workflow.Contains("E")&&!GetRequirement("CertificateSkipped");
            ...
            if(certificateRequired&&ta&&!workflow.Contains("E")&&!GetRequirement("CertificateSkipped")) btnCertificateTemplate.Text=ct?"Certificate Template ✓":"Certificate Template";
        ...
        private void StartTraining(){if(IsFeedbackRequired()&&!IsFeedbackAssigned()){pnlHostelConfirmation.Visible=false;lblMessage.ForeColor=System.Drawing.Color.Red;lblMessage.Text="Feedback is required. Please assign Feedback before starting training.";return;}if(IsCertificateRequired()&&!IsCertificateTemplateConfigured()){pnlHostelConfirmation.Visible=false;lblMessage.ForeColor=System.Drawing.Color.Red;lblMessage.Text="Certificate is required. Please configure Certificate Template before starting training.";return;}clsWorkflow.UpdateWorkflow(TrainingID,"InProgress","E");pnlHostelConfirmation.Visible=false;lblMessage.ForeColor=System.Drawing.Color.Green;lblMessage.Text="Training has started successfully.";LoadWorkflow();}
