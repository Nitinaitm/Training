...
namespace Training.Trainee
{
    public partial class MySessions : System.Web.UI.Page
    {
        ...
        private void LoadSessionDetails()
        {
            ...
            lblTrainingID.Text = Convert.ToString(d["TrainingID"]); lblCourse.Text = Convert.ToString(d["CourseName"]); lblTrainingType.Text = Convert.ToString(d["TrainingType"]); lblOrganizer.Text = Convert.ToString(d["TrainingOrganizer"]); lblSessionNo.Text = Convert.ToString(d["SessionNo"]); lblSessionName.Text = Convert.ToString(d["SessionName"]); lblTopic.Text = Convert.ToString(d["TopicName"]); lblTrainer.Text = Convert.ToString(d["TrainerName"]); DateTime sessionDate; lblSessionDate.Text = DateTime.TryParse(Convert.ToString(d["SessionDate"]), out sessionDate) ? sessionDate.ToString("dd-MMM-yyyy") : "-"; lblStartTime.Text = Convert.ToString(d["StartTime"]); lblEndTime.Text = Convert.ToString(d["EndTime"]); lblDuration.Text = d["TotalHours"] == DBNull.Value || string.IsNullOrWhiteSpace(Convert.ToString(d["TotalHours"])) ? "-" : Convert.ToString(d["TotalHours"]) + " Hours";
        }
        ...
        {
            object v = objDB.ExecuteScalar("SELECT CASE WHEN EXISTS(SELECT 1 FROM TestMaster TM INNER JOIN TestAttempt TA ON TA.TestID=TM.TestID WHERE TM.SessionID=@SessionID AND TM.TestType='Pre' AND TM.IsPublished=1 AND TA.EmpID=@EmpID AND TA.Submitted=1) THEN 1 ELSE 0 END", new SqlParameter[] { new SqlParameter("@SessionID", Session["SessionID"].ToString()), new SqlParameter("@EmpID", Session["EmpID"].ToString().ToUpperInvariant()) });
            return v != null && Convert.ToInt32(v) == 1;
        }
        ...
        protected void btnPreTest_Click(object sender, EventArgs e)
        {
            ...
            LoadRequirements();
            if (!PreRequired || PreSkipped || (AttendanceRequired && !SessionAttendanceDone())) return;
        ...
        protected void btnPostTest_Click(object sender, EventArgs e)
        {
            ...
            LoadRequirements();
            if (!PostRequired || PostSkipped || (AttendanceRequired && !SessionAttendanceDone())) return;
            if (PreRequired && !PreSkipped)
            {
                object prePublished = objDB.ExecuteScalar("SELECT CASE WHEN EXISTS(SELECT 1 FROM TestMaster WHERE SessionID=@SessionID AND TestType='Pre' AND IsPublished=1) THEN 1 ELSE 0 END", new SqlParameter[] { new SqlParameter("@SessionID", Session["SessionID"].ToString()) });
                if (prePublished != null && Convert.ToInt32(prePublished) == 1 && !IsPreCompleted()) return;
            }
