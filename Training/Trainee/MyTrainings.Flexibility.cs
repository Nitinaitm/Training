    ...
    public partial class MyTrainings
    {
        protected override void OnPreRender(EventArgs e)
        {
            ...
            for (int i = 0; i < gvTraining.Rows.Count; i++)
            {
                ...
                int sessionCount = GetCount("SELECT COUNT(*) FROM SessionMaster WHERE TrainingID=@TrainingID", trainingID, empID);
                bool attendanceDone = !attendanceRequired || (sessionCount > 0 && GetCount(@"SELECT COUNT(*) FROM SessionMaster S WHERE S.TrainingID=@TrainingID AND ISNULL(S.AttendanceSkipped,0)=0 AND NOT EXISTS (SELECT 1 FROM SessionAttendance SA WHERE SA.SessionID=S.SessionID AND SA.EmpID=@EmpID AND SA.AttendanceStatus='Completed')", trainingID, empID) == 0);

                bool preDone = !preRequired || (sessionCount > 0 && GetCount("SELECT COUNT(*) FROM SessionMaster WHERE TrainingID=@TrainingID AND ISNULL(PreAssessmentSkipped,0)=0", trainingID, empID) == 0) || (sessionCount > 0 && GetCount(@"SELECT COUNT(*) FROM SessionMaster S WHERE S.TrainingID=@TrainingID AND ISNULL(S.PreAssessmentSkipped,0)=0 AND (NOT EXISTS (SELECT 1 FROM TestMaster TM WHERE TM.SessionID=S.SessionID AND TM.TestType='Pre' AND TM.IsPublished=1) OR NOT EXISTS (SELECT 1 FROM TestMaster TM INNER JOIN TestAttempt TA ON TA.TestID=TM.TestID WHERE TM.SessionID=S.SessionID AND TM.TestType='Pre' AND TM.IsPublished=1 AND TA.EmpID=@EmpID AND TA.Submitted=1))", trainingID, empID) == 0);
                bool postDone = !postRequired || (sessionCount > 0 && GetCount("SELECT COUNT(*) FROM SessionMaster WHERE TrainingID=@TrainingID AND ISNULL(PostAssessmentSkipped,0)=0", trainingID, empID) == 0) || (sessionCount > 0 && GetCount(@"SELECT COUNT(*) FROM SessionMaster S WHERE S.TrainingID=@TrainingID AND ISNULL(S.PostAssessmentSkipped,0)=0 AND (NOT EXISTS (SELECT 1 FROM TestMaster TM WHERE TM.SessionID=S.SessionID AND TM.TestType='Post' AND TM.IsPublished=1) OR NOT EXISTS (SELECT 1 FROM TestMaster TM INNER JOIN TestAttempt TA ON TA.TestID=TM.TestID WHERE TM.SessionID=S.SessionID AND TM.TestType='Post' AND TM.IsPublished=1 AND TA.EmpID=@EmpID AND TA.Submitted=1))", trainingID, empID) == 0);
