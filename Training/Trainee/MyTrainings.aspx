...
FROM TrainingDetails TD WHERE TD.TrainingID=@TrainingID", new System.Data.SqlClient.SqlParameter[] { new System.Data.SqlClient.SqlParameter("@TrainingID", trainingID) });
        ...
        bool attendanceRequired=System.Convert.ToBoolean(r["AttendanceRequired"]);
        bool preRequired=System.Convert.ToBoolean(r["InitialAssessmentRequired"]);
        bool postRequired=System.Convert.ToBoolean(r["FinalAssessmentRequired"]);
        bool feedbackRequired=System.Convert.ToBoolean(r["FeedbackRequired"]);
        bool feedbackSkipped=System.Convert.ToBoolean(r["FeedbackSkipped"]);
        bool certificateRequired=System.Convert.ToBoolean(r["CertificateRequired"]);
        bool certificateSkipped=System.Convert.ToBoolean(r["CertificateSkipped"]);
        bool attendanceDone=true, preDone=true, postDone=true, feedbackDone=false;

        if(attendanceRequired)
            attendanceDone=System.Convert.ToInt32(objDB.ExecuteScalar(@"SELECT CASE WHEN NOT EXISTS(SELECT 1 FROM SessionMaster SM WHERE SM.TrainingID=@TrainingID AND ISNULL(SM.AttendanceSkipped,0)=0 AND ISNULL(SM.AttendanceStatus,'')<>'Completed') THEN 1 ELSE 0 END",new System.Data.SqlClient.SqlParameter[]{new System.Data.SqlClient.SqlParameter("@TrainingID",trainingID)}))==1;

        if(preRequired)
            preDone=System.Convert.ToInt32(objDB.ExecuteScalar(@"SELECT CASE WHEN NOT EXISTS(SELECT 1 FROM SessionMaster SM WHERE SM.TrainingID=@TrainingID AND ISNULL(SM.PreAssessmentSkipped,0)=0 AND (NOT EXISTS(SELECT 1 FROM TestMaster TM WHERE TM.SessionID=SM.SessionID AND TM.TestType='Pre' AND TM.IsPublished=1) OR NOT EXISTS(SELECT 1 FROM TestMaster TM INNER JOIN TestAttempt AT ON AT.TestID=TM.TestID WHERE TM.SessionID=SM.SessionID AND TM.TestType='Pre' AND TM.IsPublished=1 AND AT.EmpID=@EmpID AND AT.Submitted=1) OR (ISNULL(TD.AttendanceRequired,0)=1 AND ISNULL(SM.AttendanceSkipped,0)=0 AND ISNULL(SM.AttendanceStatus,'')<>'Completed'))) THEN 1 ELSE 0 END FROM TrainingDetails TD WHERE TD.TrainingID=@TrainingID",new System.Data.SqlClient.SqlParameter[]{new System.Data.SqlClient.SqlParameter("@TrainingID",trainingID),new System.Data.SqlParameter("@EmpID",empID)}))==1;

        if(postRequired)
            postDone=System.Convert.ToInt32(objDB.ExecuteScalar(@"SELECT CASE WHEN NOT EXISTS(SELECT 1 FROM SessionMaster SM WHERE SM.TrainingID=@TrainingID AND ISNULL(SM.PostAssessmentSkipped,0)=0 AND (NOT EXISTS(SELECT 1 FROM TestMaster TM WHERE TM.SessionID=SM.SessionID AND TM.TestType='Post' AND TM.IsPublished=1) OR NOT EXISTS(SELECT 1 FROM TestMaster TM INNER JOIN TestAttempt AT ON AT.TestID=TM.TestID WHERE TM.SessionID=SM.SessionID AND TM.TestType='Post' AND TM.IsPublished=1 AND AT.EmpID=@EmpID AND AT.Submitted=1) OR (ISNULL(TD.AttendanceRequired,0)=1 AND ISNULL(SM.AttendanceSkipped,0)=0 AND ISNULL(SM.AttendanceStatus,'')<>'Completed') OR (ISNULL(TD.InitialAssessmentRequired,0)=1 AND ISNULL(SM.PreAssessmentSkipped,0)=0 AND (NOT EXISTS(SELECT 1 FROM TestMaster TM WHERE TM.SessionID=SM.SessionID AND TM.TestType='Pre' AND TM.IsPublished=1) OR NOT EXISTS(SELECT 1 FROM TestMaster TM INNER JOIN TestAttempt AT ON AT.TestID=TM.TestID WHERE TM.SessionID=SM.SessionID AND TM.TestType='Pre' AND TM.IsPublished=1 AND AT.EmpID=@EmpID AND AT.Submitted=1)))) THEN 1 ELSE 0 END FROM TrainingDetails TD WHERE TD.TrainingID=@TrainingID",new System.Data.SqlClient.SqlParameter[]{new System.Data.SqlClient.SqlParameter("@TrainingID",trainingID),new System.Data.SqlClient.SqlParameter("@EmpID",empID)}))==1;

        if(feedbackRequired&&!feedbackSkipped)
            feedbackDone=System.Convert.ToInt32(objDB.ExecuteScalar("SELECT CASE WHEN EXISTS(SELECT 1 FROM BatchFeedback BF WHERE BF.TrainingID=@TrainingID AND BF.EmpID=@EmpID AND ISNULL(BF.Submitted,0)=1) THEN 1 ELSE 0 END",new System.Data.SqlClient.SqlParameter[]{new System.Data.SqlClient.SqlParameter("@TrainingID",trainingID),new System.Data.SqlClient.SqlParameter("@EmpID",empID)}))==1;
        ...
        if(certificate!=null)
        {
            ...
            if(!allowed) certificate.CssClass="btn btn-info btn-sm disabled";
        }
    }
}
</script>
