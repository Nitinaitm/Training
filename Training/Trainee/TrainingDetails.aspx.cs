...
namespace Training.Trainee
{
    public partial class TrainingDetails : System.Web.UI.Page
    {
        ...
        {
            string sql = "SELECT COUNT(*) TotalSession,SUM(CASE WHEN ISNULL(SA.AttendanceStatus,'Pending')='Completed' THEN 1 ELSE 0 END) AttendanceCompleted FROM SessionMaster SM LEFT JOIN SessionAttendance SA ON SA.SessionID=SM.SessionID AND SA.EmpID=@EmpID WHERE SM.TrainingID=@TrainingID";
        ...
        {
            string sql = "SELECT TD.AttendanceRequired,TD.InitialAssessmentRequired,TD.FinalAssessmentRequired,TD.FeedbackRequired,TD.CertificateRequired,ISNULL(TP.PreExamCompleted,0) AS PreExamCompleted,ISNULL(TP.PostExamCompleted,0) AS PostExamCompleted,ISNULL(TP.BatchFeedbackCompleted,0) AS BatchFeedbackCompleted,CASE WHEN NOT EXISTS (SELECT 1 FROM SessionMaster SM WHERE SM.TrainingID=@TrainingID AND ISNULL((SELECT TOP 1 AttendanceStatus FROM SessionAttendance SA WHERE SA.SessionID=SM.SessionID AND SA.EmpID=@EmpID),'Pending')<>'Completed') THEN 1 ELSE 0 END AS AttendanceDone,CASE WHEN EXISTS (SELECT 1 FROM TrainingCertificate WHERE TrainingID=@TrainingID AND EmpID=@EmpID AND CertificateStatus='A') THEN 1 ELSE 0 END AS CertificateReady FROM TrainingDetails TD LEFT JOIN TrainingProgress TP ON TP.TrainingID=TD.TrainingID AND TP.EmpID=@EmpID WHERE TD.TrainingID=@TrainingID";
            ...
            DataRow dr = dt.Rows[0]; bool attendanceRequired = Convert.ToBoolean(dr["AttendanceRequired"]), preRequired = Convert.ToBoolean(dr["InitialAssessmentRequired"]), postRequired = Convert.ToBoolean(dr["FinalAssessmentRequired"]), feedbackRequired = Convert.ToBoolean(dr["FeedbackRequired"]), certificateRequired = Convert.ToBoolean(dr["CertificateRequired"]); bool attendanceDone = Convert.ToBoolean(dr["AttendanceDone"]), preDone = Convert.ToBoolean(dr["PreExamCompleted"]), postDone = Convert.ToBoolean(dr["PostExamCompleted"]), batchFeedbackDone = Convert.ToBoolean(dr["BatchFeedbackCompleted"]), certificateReady = Convert.ToBoolean(dr["CertificateReady"]);
            ...
            bool attendanceGate = !attendanceRequired || attendanceDone; bool requiredTestsDone = (!preRequired || preDone) && (!postRequired || postDone); bool feedbackGate = !feedbackRequired || batchFeedbackDone; bool workflowComplete = certificateRequired && attendanceGate && requiredTestsDone && feedbackGate;
